...
parser.add_argument('--model-type', default='LSTMAE', help='type of model to use: LSTMAE or LSTMAE_PRED for '
                                                           'prediction')
parser.add_argument('--seq-len', default=1000, help='sequence full length')
...
parser.add_argument('--data-path', type=str, default=r'C:\Users\dwmcc\OneDrive\Documents\GitHub\AutoEncoderLSTM\square_wave_with_large_spikes.csv',
...
class square_dataset(torch.utils.data.Dataset):
    def __init__(self, df, vals_scaling_metadata, seq_len):
        self.df = df
        self.vals_scaling_metadata = vals_scaling_metadata
        self.seq_len = seq_len

    def __len__(self):
        return self.df.__len__() - (self.seq_len-1)

    def __getitem__(self, index):        
        return torch.FloatTensor(self.df[index:index+self.seq_len]).unsqueeze(dim=-1)
...
def run_experiment():
    ...
    train_vals, test_vals = get_train_test_data(df)
    ...
    for train_index, val_index in kf.split(train_vals):
        ...
        k_train_df = train_vals[train_index]
        k_val_df = train_vals[val_index]

        # Getnerate relevant data loaders
        k_train_iter = torch.utils.data.DataLoader(square_dataset(k_train_df, vals_scaling_metadata, args.seq_len), batch_size=args.batch_size, shuffle=True)
        k_val_iter = torch.utils.data.DataLoader(square_dataset(k_val_df, vals_scaling_metadata, args.seq_len), batch_size=args.batch_size, shuffle=True)
...
def get_train_test_data(df, split_ratio=0.8):
    ...
    """
    train_vals = np.array(df[args.val_type].iloc[:int(split_ratio * len(df))])
    test_vals = np.array(df[args.val_type].iloc[int(split_ratio * len(df)):])
